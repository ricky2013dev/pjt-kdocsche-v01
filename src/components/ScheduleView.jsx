...
const ScheduleView = ({ selectedSlot, onSlotSelect, onNext }) => {
  ...
  const renderMonthlyCalendar = () => {
    ...
    for (let i = 0; i < startingDayOfWeek; i++) {
      days.push(
        <div
          key={`empty-${i}`}
          className="border-2 border-gray-200 rounded-lg p-2 md:p-3 min-h-[100px] md:min-h-[120px] opacity-30 pointer-events-none"
        ></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        ...
        >
          <div className="font-bold text-gray-700 mb-1 text-xs md:text-base">
            {day}
          </div>
  ...
  const renderWeeklyCalendar = () => {
    // Show 7 days starting from currentMonth date (default is today)
    ...
    // Show all 7 days on both mobile and desktop
    const daysToShow = 7;
    ...
    for (let i = 0; i < daysToShow; i++) {
      ...
      elements.push(
        <div
          key={`header-${i}`}
          className="p-1 md:p-2 font-bold text-center border-b-2 border-gray-300 text-xs md:text-sm"
        >
  ...
  const changeMonth = (direction) => {
    ...
    if (view === "weekly") {
      // Navigate by 7 days for weekly view
      newMonth.setDate(currentMonth.getDate() + direction * 7);
    } else {
      // Navigate by month for monthly view
          ...
          <span className="font-bold text-xs md:text-lg text-center px-1">
            {view === "weekly" ? (
              <>
                ...
                {" - "}
                {new Date(
                  currentMonth.getTime() + 6 * 24 * 60 * 60 * 1000
