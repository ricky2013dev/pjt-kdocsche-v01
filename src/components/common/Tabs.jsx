      ...
      <div className="flex border-b-2 border-gray-300 mb-4 md:mb-6 overflow-x-auto -mx-2 md:mx-0 px-2 md:px-0 scrollbar-hide">
        {tabs.map((tab, index) => (
          <button
            ...
            className={`px-2 md:px-6 py-2 md:py-3 font-medium text-sm md:text-base whitespace-nowrap transition-all relative flex-shrink-0 ${
              activeTab === tab.id
          ...
          >
            ...
            {tab.required && activeTab !== tab.id && (
              <span className="ml-0.5 md:ml-1 text-red-500 text-sm md:text-base">*</span>
            )}
            {tab.completed && activeTab !== tab.id && (
              <span className="ml-1 md:ml-2 text-green-500 text-sm md:text-base">✓</span>
            )}
          </button>
        ))}
      </div>
